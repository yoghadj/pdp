...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.penanganans.update", [$penanganan->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="min">{{ trans('cruds.penanganan.fields.min') }}</label>
                <input class="form-control {{ $errors->has('min') ? 'is-invalid' : '' }}" type="number" name="min" id="min" value="{{ old('min', $penanganan->min) }}" step="1" required>
                @if($errors->has('min'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('max') ? 'is-invalid' : '' }}" type="number" name="max" id="max" value="{{ old('max', $penanganan->max) }}" step="1" required>
                @if($errors->has('max'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('penanganan') ? 'is-invalid' : '' }}" type="text" name="penanganan" id="penanganan" value="{{ old('penanganan', $penanganan->penanganan) }}" required>
