...
@can('penanganan_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.penanganans.create') }}">
                ...
                <tbody>
                    @foreach($penanganans as $key => $penanganan)
                        <tr data-entry-id="{{ $penanganan->id }}">
                            ...
                            <td>
                                @can('penanganan_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.penanganans.show', $penanganan->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('penanganan_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.penanganans.edit', $penanganan->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('penanganan_delete')
                                    <form action="{{ route('admin.penanganans.destroy', $penanganan->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('penanganan_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.penanganans.massDestroy') }}",
