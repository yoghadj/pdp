...
@can('assess_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.assesses.create') }}">
                ...
                <tbody>
                    @foreach($assesses as $key => $assess)
                        <tr data-entry-id="{{ $assess->id }}">
                            ...
                            <td>
                                @can('assess_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.assesses.show', $assess->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('assess_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.assesses.edit', $assess->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('assess_delete')
                                    <form action="{{ route('admin.assesses.destroy', $assess->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('assess_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.assesses.massDestroy') }}",
