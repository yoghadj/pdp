...
@can('cenderung_dampak_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.cenderung-dampaks.create') }}">
                        ...
                        <tr data-entry-id="{{ $cenderungDampak->id }}">
                            ...
                            <td>
                                @can('cenderung_dampak_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.cenderung-dampaks.show', $cenderungDampak->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('cenderung_dampak_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.cenderung-dampaks.edit', $cenderungDampak->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('cenderung_dampak_delete')
                                    <form action="{{ route('admin.cenderung-dampaks.destroy', $cenderungDampak->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('cenderung_dampak_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.cenderung-dampaks.massDestroy') }}",
