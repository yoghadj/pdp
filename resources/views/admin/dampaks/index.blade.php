...
@can('dampak_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.dampaks.create') }}">
                ...
                <tbody>
                    @foreach($dampaks as $key => $dampak)
                        <tr data-entry-id="{{ $dampak->id }}">
                            ...
                            <td>
                                @can('dampak_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.dampaks.show', $dampak->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('dampak_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.dampaks.edit', $dampak->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('dampak_delete')
                                    <form action="{{ route('admin.dampaks.destroy', $dampak->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('dampak_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.dampaks.massDestroy') }}",
