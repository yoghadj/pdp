...
@can('ropa_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.ropas.create') }}">
                ...
                <tbody>
                    @foreach($ropas as $key => $ropa)
                        <tr data-entry-id="{{ $ropa->id }}">
                            ...
                            <td>
                                @can('ropa_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.ropas.show', $ropa->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('ropa_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.ropas.edit', $ropa->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('ropa_delete')
                                    <form action="{{ route('admin.ropas.destroy', $ropa->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('ropa_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.ropas.massDestroy') }}",
